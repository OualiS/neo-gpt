...
class NNLP {
  ...
  process(_utterance) {
    ...
    loop1: for (let loop_intent_name in this.Intents) {
      for (
        let a = 0;
        a < this.Intents[loop_intent_name].Documents.length;
        a++
      ) {
        const RESULT = this._process(
          this.Intents[loop_intent_name].Documents[a],
          SPLITTED_SAY
        );
        if (RESULT !== false) {
    ...
    if (this.Intents[intent_name].Actions.length > 0) {
      for (
        let index = 0;
        index < this.Intents[intent_name].Actions.length;
        index++
      ) {
        this.Intents[intent_name].Actions[index](
  ...
  _checkIntentExistence(_intentName) {
    if (this.Intents[_intentName] === undefined) {
      this.Intents[_intentName] = new Intent(_intentName);
    }
  }
  ...
  _process(_sentence, _splitedSay, _asc = true) {
    ...
    loop3: for (let b = 0; b < SPLITTED_SENTENSE.length; b++) {
      // SI LE MOT EST UNE VARIABLE
      if (
        SPLITTED_SENTENSE[b][0] === "{" &&
        SPLITTED_SENTENSE[b][SPLITTED_SENTENSE[b].length - 1] === "}"
      ) {
        let without_percent = SPLITTED_SENTENSE[b]
          .substring(1, SPLITTED_SENTENSE[b].length - 1)
          .split("|");
        ...
        while (
          _splitedSay[b + variables_position] !== next_word &&
          _splitedSay[b + variables_position] !== undefined
        ) {
